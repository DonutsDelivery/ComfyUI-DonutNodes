...
def _first_existing_symlink(base_dir: str, target_dir: str):
    """Return the first existing symlink between base_dir and target_dir."""
    base_abs = os.path.abspath(base_dir)
...
def _next_counter(folder: str, filename: str) -> int:
    """Match Donut's ``name_00001_.safetensors`` numbering."""
...
def get_model_save_path(folder_paths_module, filename_prefix: str, output_dir: str):
    """Resolve a Donut model-save destination.

    Returns the same tuple shape as ``folder_paths.get_save_image_path``. Normal
    ComfyUI behavior always wins. The fallback exists solely for an intentional,
    already-existing symlink contained lexically under ``output_dir``.
    """
    try:
        ...
        # Do not try to reinterpret ComfyUI's %-variable filename syntax in the
        # fallback. If a caller uses it, retain ComfyUI's original validation.
        ...
        if symlink is None:
            # The failure was not caused by an intentional symlink under output;
            # preserve ComfyUI's normal error rather than weakening validation.
