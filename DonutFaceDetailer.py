...
def _ensure_impact():
    """Resolve Impact at execution time if its pack loaded after DonutNodes."""
    global IMPACT_AVAILABLE, core, impact_wildcards, impact_sampling, impact_utils, nodes_differential_diffusion
    if IMPACT_AVAILABLE:
        return
    try:
        ...
        raise RuntimeError(
            "DonutFaceDetailer requires a working ComfyUI Impact Pack. "
            "Install/repair it through Manager and restart ComfyUI."
        ) from error
    IMPACT_AVAILABLE = True


# Take advantage of already loaded dependencies, but never gate registration on
# filesystem traversal order. ComfyUI finishes loading all packs before running.
try:
    _ensure_impact()
except RuntimeError:
    pass
...
class DonutFaceDetailer:
    ...
    def enhance_detail_megapixel(
        ...
        if edit_mode:
            ...
            sampling_model, sampling_positive, sampling_negative, source_latent, _ = prepare_krea2_edit(
