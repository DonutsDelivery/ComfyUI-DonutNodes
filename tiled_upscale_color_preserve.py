...
This patches the existing node class in-place instead of registering a second node.
The upscaler still returns IMAGE outputs; color preservation is applied to the final
sampled/decoded image using reference-image channel statistics.
"""
...
def patch_tiled_upscale_color_preserve():
    """Patch DonutTiledUpscale in-place once."""
    ...
    def input_types(cls):
        ...
        optional["color_preserve_strength"] = (
            "FLOAT",
            {
                ...
                "max": 1.0,
                "step": 0.05,
                "tooltip": "0 disables color preservation; 1 fully matches RGB mean/std to the reference after sampling and decode.",
            },
        )
        return inputs
...
patch_tiled_upscale_color_preserve()
